...
// Pattern: File Upload Component — ngx-dropzone wrapper, FileDropZoneData model, onFileSelected callback pattern,
// initFileData (mevcut dosyalar), onFileUploaded (başarı), onFileUploadedError (hata), onRemove, getInsertedFiles, getDeletedFiles
...
export class PixdinnDropzoneComponent implements OnInit {
  ...
  // Mevcut (edit modunda) dosyaları yüklemek için
  ...
  // ngx-dropzone'dan dosya seçildiğinde
  onSelect(event) {
    ...
    event.addedFiles.forEach(element => {
      ...
      if (this.onFileSelected != null) {
        this.onFileSelected(fileDrop);  // Parent component'e callback
      }
    });
  }

  // Upload yeniden başlatma
  ...
  // Upload başarıyla tamamlandığında
  ...
  // Başarıyla yüklenen yeni dosyaları getirir
  getInsertedFiles() {
    return this.fileData.filter(f => f.oldfile == false && f.onUploaded == true);
  }

  // Silinen dosyaları getirir
  ...
  getfileType(file: File): DropZoneFiletype {
    ...
    if (file.type.startsWith("image")) {
      dropzoneType = DropZoneFiletype.DropImage;
    } else if (file.type.startsWith("application/pdf")) {
      dropzoneType = DropZoneFiletype.DropPdf;
    }
    return dropzoneType;
  }

  getFileUrl(type: DropZoneFiletype, url: string): string {
    if (type == DropZoneFiletype.DropImage) {
      return url;
    } else if (type == DropZoneFiletype.DropPdf) {
      return DropZoneTempImage.Pdf;
    } else if (type == DropZoneFiletype.DropOther) {
