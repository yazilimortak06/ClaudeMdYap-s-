// Kaynak: E:\Projeler\Angular\EvTechPanelAltunkaya\src\app\shared_admin\partials\subheader\_services\subheader.service.ts
...
export class SubHeaderService implements OnDestroy {
  titleSubject: BehaviorSubject<string> = new BehaviorSubject<string>('Panel');
  ...
  subheaderVersionSubject: BehaviorSubject<string> = new BehaviorSubject<string>('v1');
  ...
  setTitle(title: string = '') { this.titleSubject.next(title); }
  setDescription(description: string) { this.descriptionSubject.next(description); }
  private setSubheaderVersion(version: string = 'v1') { this.subheaderVersionSubject.next(version); }
  ...
  private getLinksFromMenu(menu): HTMLAnchorElement[] {
    ...
    parentLiElements.forEach((el) => {
      ...
      if (links && links.length > 0) {
        const aLink = links[0];
        if (aLink.href && aLink.href.length > 0 && aLink.innerHTML !== '/') { result.push(aLink); }
      }
    });
  ...
  private getBreadcrumbsAndTitle(menuId, pathName): SubheaderModel {
    ...
    activeLinks.forEach((link) => {
      ...
      if (titleSpans) {
        ...
        if (titleSpan) {
          result.breadcrumbs.push({ title: titleSpan.innerHTML, linkPath: link.pathname, linkText: titleSpan.innerHTML, isActive: null });
        }
      }
    });
